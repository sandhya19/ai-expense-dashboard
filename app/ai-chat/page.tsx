...
export default async function AiChatPage({ searchParams }: { searchParams: Promise<{ demo?: string }> }) {
  const demoMode = (await searchParams).demo === "1";
  const receipts = await getReceiptList(demoMode);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          {demoMode ? "Ask questions about the fictional Demo Mode receipt history." : "Ask ReceiptBrain questions about your receipt history and spending patterns."}
        </p>
      </div>

      <ReceiptChat receipts={receipts} demoMode={demoMode} />
